...
def extract_from_to(text: str):
    ...
    """
    text = text.replace('back to','to').replace('From', 'from')
...
def is_reasonalbe_visiting_city(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if 'from' in city_value or 'From' in city_value:
    ...
    if city_list[0] != city_list[-1]:
        return False, "The transportation should be a closed circle."
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if 'breakfast' in unit and unit['breakfast'] and unit['breakfast'] != '-':
            if unit['breakfast'] not in restaurants_list:
                ...
                return False, f"The restaurant in day {i+1} breakfast({unit['breakfast']}) is repeated."
        ...
        if 'dinner' in unit and unit['dinner'] and unit['dinner'] != '-':
            if unit['dinner'] not in restaurants_list:
                ...
                return False, f"The restaurant in day {i+1} dinner({unit['dinner']}) is repeated."
...
def is_valid_transportation(question, tested_data):
    ...
    if (('Self-driving' in transportation_list) and ('Flight' in transportation_list)) or (('Taxi' in transportation_list) and ('Self-driving' in transportation_list)):
        return False, "The transportation is conflicting: self-driving and other modes of transport cannot be used together."
...
def is_valid_information_in_current_city(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if 'from' in current_city or 'From' in current_city:
        ...
        if 'transportation' in unit and unit['transportation'] and unit['transportation'] != '-':
            for city in final_city_list:
                if city not in unit['transportation']:
                    return False, f"The transportation in day {i+1}({unit['transportation']}) is invalid city choice."
        # elif 'transportation' not in unit:
        #     return False, f"No Transportation Info."
        ...
        if 'breakfast' in unit and unit['breakfast'] and unit['breakfast'] != '-':
            ...
            if not flag:
                return False, f"The breakfast in day {i+1}({unit['breakfast']}) is invalid city choice."
        # elif 'breakfast' not in unit:
        #     return False, f"No Breakfast Info."
        ...
        if 'lunch' in unit and unit['lunch'] and unit['lunch'] != '-':
            ...
            if not flag:
                return False, f"The lunch in day {i+1}({unit['lunch']}) is invalid city choice."
        # elif 'lunch' not in unit:
        #     return False, f"No Lunch Info."
        ...
        if 'dinner' in unit and unit['dinner'] and unit['dinner'] != '-':
            ...
            if not flag:
                return False, f"The dinner in day {i+1}({unit['dinner']}) is invalid city choice."
        # elif 'dinner' not in unit:
        #     return False, f"No Dinner Info."
            ...
            for attraction in attraction_list:
                ...
                for city in final_city_list:
                    if city  in attraction:
                        flag = True
                if not flag:
                    return False, f"The attraction in day {i+1}({unit['attraction']}) is invalid city choice."
        ...
        if 'accommodation' in unit and unit['accommodation'] and unit['accommodation'] != '-':
            ...
            if final_city_list[-1] not in unit['accommodation']:
                return False, f"The accommodation in day {i+1}({unit['accommodation']}) is invalid city choice."
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if 'breakfast' in unit and unit['breakfast'] and unit['breakfast'] != '-':
            ...
            if len(restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]) < 1:
                return False, f"The breakfast in day {i+1}({unit['breakfast']}) is invalid in the sandbox."
        # elif 'breakfast' not in unit:
        #     return False, f"No Breakfast Info."
        ...
        if 'lunch' in unit and unit['lunch'] and unit['lunch'] != '-':
            ...
            if len(restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]) < 1:
                return False, f"The lunch in day {i+1}({unit['lunch']}) is invalid in the sandbox."
        # elif 'lunch' not in unit:
        #     return False, f"No Lunch Info."
        ...
        if 'dinner' in unit and unit['dinner'] and unit['dinner'] != '-':
            ...
            if len(restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]) < 1:
                return False, f"The dinner in day {i+1}({unit['dinner']}) is invalid in the sandbox."
        # elif 'dinner' not in unit:
        #     return False, f"No Dinner Info."
        ...
        if 'attraction' in unit and unit['attraction'] and unit['attraction'] != '-':
            ...
            for attraction in attractions_list:
                ...
                if len(attractions.data[(attractions.data['Name'].astype(str).str.contains(re.escape(name))) & (attractions.data['City'] == city)]) < 1:
                    return False, f"The attraction {attraction} in day {i+1}({unit['attraction']}) is invalid in the sandbox."
        # elif 'attraction' not in unit:
        #     return False, f"No Attraction Info."
        ...
        if 'accommodation' in unit and unit['accommodation'] and unit['accommodation'] != '-':
            ...
            if len(accommodation.data[(accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (accommodation.data['city'] == city)]) < 1:
                return False, f"The accommodation in day {i+1}({unit['accommodation']}) is invalid in the sandbox."
        # elif 'accommodation' not in unit:
        #     return False, f"No Accommodation Info."
...
def is_not_absent(question, tested_data):
    ...
    if not is_valid_visiting_city_number(question, tested_data)[0]:
        return False, is_valid_visiting_city_number(question, tested_data)[1]
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if ('from ' in unit['current_city'].lower() or 'to ' in unit['current_city']) and unit['transportation'] in ['','-']:
            return False, f"No transportation in day {i+1}({unit['current_city']}) is not allowed."
        
        if ('from ' not in unit['current_city'].lower() and  ' to ' not in unit['current_city']) and unit['attraction'] in ['','-']:
            return False, f"No attaction in day {i+1}({unit['current_city']}) is not allowed."

        if i != question['days'] - 1 and unit['accommodation'] in ['','-']:
            return False, f"No accommodation in day {i+1}({unit['current_city']}) is not allowed."

        if (unit['breakfast'] in ['','-'] or unit['lunch'] in ['','-'] or unit['dinner'] in ['','-']) and 'from ' not in unit['current_city'].lower():
            return False, f"No restaurant in day {i+1}({unit['current_city']}) is not allowed."
