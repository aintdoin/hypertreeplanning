    ...
    for i in range(min(question['days'],len(tested_data))):
            ...
            if org_city == None or dest_city == None:
                ...
                if 'flight number' in value.lower():
                    ...
                    if len(res) > 0:
                        ...
                        if res['Price'].values[0] * question['people_number']>max_cost:
                            max_cost = res['Price'].values[0] * question['people_number']
                            max_cost_name = res.to_string()
                    ...
                    if 'self-driving' in value.lower():
                        ...
                        if cost * math.ceil(question['people_number'] * 1.0 / 5)>max_cost:
                            max_cost = cost * math.ceil(question['people_number'] * 1.0 / 5)
                            max_cost_name = f'self-driving from {org_city} to {dest_city}'
                    else:
                        ...
                        if cost * math.ceil(question['people_number'] * 1.0 / 4)>max_cost:
                            max_cost = cost * math.ceil(question['people_number'] * 1.0 / 4)
                            max_cost_name = f'taxi from {org_city} to {dest_city}'
        ...
        if unit['breakfast'] and unit['breakfast'] != '-':
            ...
            if len(res) > 0:
                ...
                if res['Average Cost'].values[0] * question['people_number']>max_cost:
                    max_cost = res['Average Cost'].values[0] * question['people_number']
                    max_cost_name = f"restaurant({unit['breakfast']})"
        ...
        if unit['lunch'] and unit['lunch'] != '-':
            ...
            if len(res) > 0:
                ...
                if res['Average Cost'].values[0] * question['people_number']>max_cost:
                    max_cost = res['Average Cost'].values[0] * question['people_number']
                    max_cost_name = f"restaurant({unit['lunch']})"
        ...
        if unit['dinner'] and unit['dinner'] != '-':
            ...
            if len(res) > 0:
                ...
                if res['Average Cost'].values[0] * question['people_number']>max_cost:
                    max_cost = res['Average Cost'].values[0] * question['people_number']
                    max_cost_name = f"restaurant({unit['dinner']})"
        ...
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            if len(res) > 0:
                ...
                if res['price'].values[0] * math.ceil(question['people_number'] * 1.0 / res['maximum occupancy'].values[0])>max_cost:
                    max_cost = res['price'].values[0] * math.ceil(question['people_number'] * 1.0 / res['maximum occupancy'].values[0])
                    max_cost_name = f"accommodation({unit['accommodation']})"
    print(total_cost)
    return total_cost,max_cost_name
        ...
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            if len(res) > 0:
                if question['local_constraint']['house rule'] == 'smoking' and 'No smoking' in str(res['house_rules'].values[0]):
                    return False, f"The house rule should be {question['local_constraint']['house rule']}, but the accommodation({unit['accommodation']}) does not meet this condition."
        ...
        if len(cuisine_set) == len(question['local_constraint']['cuisine']):
            ...
            for cuisine in question['local_constraint']['cuisine']:
                if cuisine not in cuisine_set:
                    return False, f"The cuisine {cuisine} is not satisfied in all restaurants."
        ...
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            if len(res) > 0:
                if question['local_constraint']['room type'] == 'not shared room' and res['room type'].values[0] == 'Shared room':
                    return False, f"The room type should be {question['local_constraint']['room type']}, but the accommodation({unit['accommodation']}) does not meet this condition."
...
def evaluation(query_data, tested_data):
    ...
    total_cost,max_cost_name = get_total_cost(query_data, tested_data)
    return_info['valid_cost'] = (bool(total_cost<= query_data['budget']), None if bool(total_cost<= query_data['budget']) else f"The most expensive cost is: {max_cost_name}")
    return return_info
